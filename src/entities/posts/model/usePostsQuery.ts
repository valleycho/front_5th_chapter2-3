...
export const useGetPostsQuery = (allUser?: AllUserResponse) => {
  ...
  const { limit, skip, searchQuery } = useQueryParamsHook()

  return useQuery({
    ...postKeys.list({ limit, skip, searchQuery, selectedTag: searchParams.get("tag") || "" }, allUser),
    enabled: !!allUser,
  })
}
...
export const useDeletePostMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: (postResponse: PostType) => {
      queryClient.setQueryData(postKeys.list({ limit, skip, searchQuery, selectedTag: searchParams.get("tag") || "" }).queryKey, (old: PostResponse) => {
        return {
          ...old,
